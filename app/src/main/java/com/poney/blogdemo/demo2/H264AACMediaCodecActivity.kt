...
class H264AACMediaCodecActivity : AppCompatActivity() {
    ...
    private fun startMediaCodecRecord(width: Int?, height: Int?, data: ByteArray?) {
        if (mMediaCodecEncoder == null) {
            mMediaCodecEncoder = H264MediaCodecEncoder(width!!, height!!)
            mMediaCodecEncoder?.setOutputPath(externalCacheDir?.absolutePath + File.separator + "mediacodec_demo.mp4")
